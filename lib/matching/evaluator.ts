...
function evaluateChoiceQuestion(question: UiQuestion, rawAnswer: unknown): boolean | null {
  ...
  const rule = question.clause?.rule as { operator?: string; value?: unknown } | undefined;
  const operator = (rule?.operator ?? "equals").toString().toLowerCase();
  ...
  if (normalizedRuleValues.length === 0) {
    // No specific values to compare against — we can't evaluate this criterion.
    // Return null (unknown) rather than true to avoid falsely disqualifying patients
    // on exclusion choice questions where any answer would otherwise trigger "no".
    return null;
  }

  if (operator === "any_except") {
...
function evaluateQuestion(question: UiQuestion, answers: AnswerMap): boolean | null {
  ...
  if (kind === "boolean") {
    ...
    const expected = normalizeChoiceValue(rule?.value);
    if (expected) {
      const actual = normalizeChoiceValue(response);
