-- LEGACY: This schema reflects an older version of the project. 
-- It may not match the current Supabase project used by the storefront.
-- See docs/serving_contract.md for the current source of truth for serving tables.
...
CREATE OR REPLACE FUNCTION "public"."nearest_sites"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision DEFAULT NULL::double precision) RETURNS TABLE("trial_id" "uuid", "nct_id" "text", "nearest_miles" double precision)
    LANGUAGE "sql" STABLE
    AS $$
  select
    s.trial_id,
    ...
    min(
      3959 * 2 * asin(
        sqrt(
          sin(radians((in_lat - s.lat)/2))^2
          + cos(radians(in_lat)) * cos(radians(s.lat))
          * sin(radians((in_lon - s.lon)/2))^2
        )
      )
    ) as nearest_miles
  from public.sites s
  where s.lat is not null and s.lon is not null
  group by s.trial_id, s.nct_id
  having max_miles is null
     or min(
          3959 * 2 * asin(
            sqrt(
              sin(radians((in_lat - s.lat)/2))^2
...
ALTER FUNCTION "public"."nearest_sites"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."nearest_sites_with_meta"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision DEFAULT NULL::double precision) RETURNS TABLE("trial_id" "uuid", "nct_id" "text", "city" "text", "state" "text", "lat" double precision, "lon" double precision, "nearest_miles" double precision)
    LANGUAGE "sql" STABLE
    AS $$
with u as (
...
CREATE OR REPLACE FUNCTION "public"."normalize_phase_value"("p" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    AS $$
  select case
    when p is null then null
    when lower(trim(p)) in ('n/a','na','not applicable') then 'NA'
    when lower(trim(p)) = 'early phase 1' then 'EARLY_PHASE1'
    when lower(trim(p)) = 'phase 1' then 'PHASE1'
    when lower(trim(p)) in ('phase 1/phase 2','phase1/phase2','phase 1 / phase 2') then 'PHASE1/PHASE2'
    when lower(trim(p)) = 'phase 2' then 'PHASE2'
    when lower(trim(p)) in ('phase 2/phase 3','phase2/phase3','phase 2 / phase 3') then 'PHASE2/PHASE3'
    when lower(trim(p)) = 'phase 3' then 'PHASE3'
    when lower(trim(p)) = 'phase 4' then 'PHASE4'
    else upper(regexp_replace(p, '\s+', ' ', 'g'))
  end
$$;


ALTER FUNCTION "public"."normalize_phase_value"("p" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."trials_phase_normalizer"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
declare
  has_drug_like boolean;
begin
  -- observational => NA
  if new.study_type = 'OBSERVATIONAL' then
    new.phase := 'NA';
  end if;

  -- normalize provided phase token
  new.phase := public.normalize_phase_value(new.phase);

  -- if interventional and phase is null/empty, infer NA when only non-drug interventions are present
  if new.study_type = 'INTERVENTIONAL' and (new.phase is null or new.phase = '') then
    -- interventions is jsonb array with objects having a "type" field (upper in your dataset)
    -- detect any drug-like types
    has_drug_like := exists (
      select 1
      from jsonb_array_elements(coalesce(new.interventions, '[]'::jsonb)) as e(elem)
      where upper(coalesce(e.elem->>'type','')) in ('DRUG','BIOLOGICAL','GENETIC','COMBINATION PRODUCT')
    );

    if not has_drug_like then
      new.phase := 'NA';
    end if;
  end if;
...
ALTER FUNCTION "public"."trials_phase_normalizer"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    "condition_slugs" "text"[] DEFAULT '{}'::"text"[],
    ...
    CONSTRAINT "profiles_travel_miles_check" CHECK ((("travel_miles" >= 0) AND ("travel_miles" <= 300)))
);
...
CREATE OR REPLACE FUNCTION "public"."upsert_profile"("p_user_id" "uuid", "p_profile" "jsonb") RETURNS "public"."profiles"
    LANGUAGE "sql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  insert into public.profiles (user_id, age, sex, zip, prefers_remote, travel_miles, condition_slugs, alert_opt_in)
  values (
    p_user_id,
    (p_profile->>'age')::smallint,
    p_profile->>'sex',
    p_profile->>'zip',
    coalesce((p_profile->>'prefers_remote')::boolean, false),
    nullif(p_profile->>'travel_miles','')::integer,
    coalesce(string_to_array(p_profile->>'condition_slugs', ','), '{}')::text[],
    coalesce((p_profile->>'alert_opt_in')::boolean, false)
  )
  on conflict (user_id) do update set
    ...
    travel_miles = excluded.travel_miles,
    condition_slugs = excluded.condition_slugs,
    alert_opt_in = excluded.alert_opt_in
  returning *;
$$;


ALTER FUNCTION "public"."upsert_profile"("p_user_id" "uuid", "p_profile" "jsonb") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."trials" (
    ...
    "sponsor_type" "text",
    "site_count" integer,
    "criteria_json" "jsonb", -- LEGACY: Replaced by questionnaire_json in serving layer
    "criteria_json_hash" "text", -- LEGACY
    "last_update_date" "date",
    "created_at" timestamp with time zone DEFAULT "now"(),
    "visit_model" "text",
    "travel_stipend" boolean,
    "accessibility_tags" "text"[],
    "detailed_description" "text",
    "condition_slugs" "text"[] DEFAULT '{}'::"text"[],
    "original_conditions" "jsonb" DEFAULT '[]'::"jsonb",
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "is_us" boolean DEFAULT false,
    "eligibility_missing" boolean DEFAULT false,
    "is_publishable" boolean DEFAULT false,
    "quality_flags" "jsonb" DEFAULT '[]'::"jsonb",
    "questionnaire_json" "jsonb", -- NEW: Primary serving contract (PMQ v10)
    CONSTRAINT "trials_visit_model_check" CHECK (("visit_model" = ANY (ARRAY['on_site'::"text", 'hybrid'::"text", 'remote'::"text"])))
);
...
CREATE OR REPLACE VIEW "public"."interventional_na_ncts" WITH ("security_invoker"='on') AS
 SELECT "nct_id",
    "title",
    "last_update_date"
   FROM "public"."trials"
  WHERE (("study_type" = 'INTERVENTIONAL'::"text") AND ("phase" = 'NA'::"text"))
  ORDER BY "last_update_date" DESC NULLS LAST, "nct_id";


ALTER VIEW "public"."interventional_na_ncts" OWNER TO "postgres";
...
ALTER TABLE "public"."patient_leads" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."physician_referrals" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    ...
    "notes" "text",
    "has_patient_consent" boolean DEFAULT false NOT NULL,
    "status" "text" DEFAULT 'received'::"text" NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "physician_referrals_status_check" CHECK (("status" = ANY (ARRAY['received'::"text", 'contacted'::"text", 'closed'::"text"])))
);


ALTER TABLE "public"."physician_referrals" OWNER TO "postgres";
...
ALTER TABLE "public"."saved_trials" OWNER TO "postgres";
...
ALTER TABLE "public"."screener_sessions" OWNER TO "postgres";
...
-- NEW: Serving table for trial locations
CREATE TABLE IF NOT EXISTS "public"."trial_sites" (
    "nct_id" "text" NOT NULL,
    "facility_name" "text",
    "city" "text",
    "state" "text",
...
CREATE TABLE IF NOT EXISTS "public"."trial_submissions" (
    ...
    "sites_json" "jsonb" NOT NULL,
    "travel_stipend" "text" NOT NULL,
    "visit_model" "text" NOT NULL,
    "inclusion_text" "text",
    "exclusion_text" "text",
    ...
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "trial_submissions_contact_method_check" CHECK (("contact_method" = ANY (ARRAY['email'::"text", 'webhook'::"text"]))),
    CONSTRAINT "trial_submissions_status_check" CHECK (("status" = ANY (ARRAY['new'::"text", 'validated'::"text", 'rejected'::"text"]))),
    CONSTRAINT "trial_submissions_travel_stipend_check" CHECK (("travel_stipend" = ANY (ARRAY['yes'::"text", 'no'::"text", 'unsure'::"text"]))),
    CONSTRAINT "trial_submissions_visit_model_check" CHECK (("visit_model" = ANY (ARRAY['on_site'::"text", 'hybrid'::"text", 'remote'::"text"])))
);


ALTER TABLE "public"."trial_submissions" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."trials_display" AS
 SELECT "id",
    "nct_id",
    "title",
    "sponsor",
    ...
    "updated_at",
    "is_us",
    COALESCE(NULLIF(TRIM(BOTH FROM "detailed_description"), ''::"text"), "brief_summary") AS "detailed_for_ui"
   FROM "public"."trials";


ALTER VIEW "public"."trials_display" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."trials_public" AS
 SELECT "id",
    "nct_id",
    "title",
    "sponsor",
...
ALTER VIEW "public"."trials_public" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."trials_quality" AS
 WITH "base" AS (
         SELECT "t"."id",
            "t"."nct_id",
            "t"."title",
            ...
            "t"."site_count",
            "t"."criteria_json",
            "t"."criteria_json_hash",
            "t"."last_update_date",
            "t"."created_at",
            "t"."visit_model",
            "t"."travel_stipend",
            "t"."accessibility_tags",
            "t"."detailed_description",
            "t"."condition_slugs",
            "t"."original_conditions",
            "t"."updated_at",
            "t"."is_us",
            "t"."eligibility_missing",
            "t"."is_publishable",
            "t"."quality_flags",
                CASE
                    WHEN ("jsonb_typeof"("t"."criteria_json") = 'array'::"text") THEN ( WITH "elems" AS (
                             SELECT "jsonb_array_elements"("t"."criteria_json") AS "elem"
                            )
                     SELECT "jsonb_build_object"('inclusion', COALESCE(( SELECT "jsonb_agg"("e"."elem") AS "jsonb_agg"
                               FROM "elems" "e"
                              WHERE (("e"."elem" ->> 'type'::"text") ~~* 'inclusion'::"text")), '[]'::"jsonb"), 'exclusion', COALESCE(( SELECT "jsonb_agg"("e"."elem") AS "jsonb_agg"
                               FROM "elems" "e"
                              WHERE (("e"."elem" ->> 'type'::"text") ~~* 'exclusion'::"text")), '[]'::"jsonb")) AS "jsonb_build_object")
                    WHEN ("jsonb_typeof"("t"."criteria_json") = 'object'::"text") THEN "t"."criteria_json"
                    ELSE '{}'::"jsonb"
                END AS "criteria_norm",
            "regexp_replace"("replace"("upper"(COALESCE("t"."status", ''::"text")), ','::"text", ''::"text"), '\s+'::"text", ' '::"text", 'g'::"text") AS "status_norm",
            "upper"(COALESCE("t"."gender", ''::"text")) AS "gender_norm"
 ...
 SELECT "id",
    ...
    "status_norm",
    "gender_norm",
    (("title" IS NULL) OR (TRIM(BOTH FROM "title") = ''::"text")) AS "qf_title_missing",
    (("sponsor" IS NULL) OR (TRIM(BOTH FROM "sponsor") = ''::"text")) AS "qf_sponsor_missing",
    (("conditions" IS NULL) OR ("array_length"("conditions", 1) IS NULL)) AS "qf_conditions_missing",
    (("locations" IS NULL) OR ("jsonb_array_length"("locations") = 0)) AS "qf_locations_empty",
    ((COALESCE("jsonb_array_length"(("criteria_norm" -> 'inclusion'::"text")), 0) = 0) AND (COALESCE("jsonb_array_length"(("criteria_norm" -> 'exclusion'::"text")), 0) = 0)) AS "qf_criteria_empty",
    false AS "qf_criteria_no_arrays",
    ("criteria_json" ? 'error'::"text") AS "qf_parser_error",
    ("gender_norm" <> ALL (ARRAY['ALL'::"text", 'MALE'::"text", 'FEMALE'::"text"])) AS "qf_gender_bad",
    (("min_age_years" IS NOT NULL) AND ("max_age_years" IS NOT NULL) AND ("min_age_years" > "max_age_years")) AS "qf_age_inverted",
    ("status_norm" <> ALL (ARRAY['NOT YET RECRUITING'::"text", 'RECRUITING'::"text", 'ENROLLING BY INVITATION'::"text", 'ACTIVE NOT RECRUITING'::"text", 'COMPLETED'::"text", 'SUSPENDED'::"text", 'TERMINATED'::"text", 'WITHDRAWN'::"text", 'UNKNOWN STATUS'::"text"])) AS "qf_status_bad",
    (("is_us" = true) AND (("location_countries" IS NULL) OR (NOT ('United States'::"text" = ANY ("location_countries"))))) AS "qf_is_us_mismatch",
    (("site_count" IS NOT NULL) AND ("site_count" <> COALESCE("jsonb_array_length"("locations"), 0))) AS "qf_sitecount_mismatch",
    (("now"() - COALESCE("updated_at", "created_at")) > '30 days'::interval) AS "qf_stale"
   FROM "base" "b";


ALTER VIEW "public"."trials_quality" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."trials_with_normalized_criteria" AS
 SELECT "id",
    "nct_id",
    "title",
    "sponsor",
    ...
    "quality_flags",
        CASE
            WHEN ("jsonb_typeof"("criteria_json") = 'array'::"text") THEN ( WITH "elems" AS (
                     SELECT "jsonb_array_elements"("t"."criteria_json") AS "elem"
                    )
             SELECT "jsonb_build_object"('inclusion', COALESCE(( SELECT "jsonb_agg"("e"."elem") AS "jsonb_agg"
                       FROM "elems" "e"
                      WHERE (("e"."elem" ->> 'type'::"text") ~~* 'inclusion'::"text")), '[]'::"jsonb"), 'exclusion', COALESCE(( SELECT "jsonb_agg"("e"."elem") AS "jsonb_agg"
                       FROM "elems" "e"
                      WHERE (("e"."elem" ->> 'type'::"text") ~~* 'exclusion'::"text")), '[]'::"jsonb")) AS "jsonb_build_object")
            WHEN ("jsonb_typeof"("criteria_json") = 'object'::"text") THEN "criteria_json"
            ELSE '{}'::"jsonb"
        END AS "criteria_norm"
   FROM "public"."trials" "t";


ALTER VIEW "public"."trials_with_normalized_criteria" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."us_place_centroids" (
    "city" "text" NOT NULL,
    "state" "text" NOT NULL,
...
ALTER TABLE "public"."us_place_centroids" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."us_place_centroids_stage" (
    "city" "text",
    "state" "text",
    "lat" double precision,
    "lon" double precision,
...
ALTER TABLE "public"."us_place_centroids_stage" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."user_digest_log" (
    "user_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."user_digest_log" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."geocode_cache"
    ADD CONSTRAINT "geocode_cache_pkey" PRIMARY KEY ("key");
...
ALTER TABLE ONLY "public"."patient_leads"
    ADD CONSTRAINT "patient_leads_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."physician_referrals"
    ADD CONSTRAINT "physician_referrals_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."saved_trials"
    ADD CONSTRAINT "saved_trials_pkey" PRIMARY KEY ("user_id", "nct_id");



ALTER TABLE ONLY "public"."screener_sessions"
    ADD CONSTRAINT "screener_sessions_pkey" PRIMARY KEY ("session_id");
...
ALTER TABLE ONLY "public"."trial_submissions"
    ADD CONSTRAINT "trial_submissions_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."trials"
    ADD CONSTRAINT "trials_nct_id_key1" UNIQUE ("nct_id");



ALTER TABLE ONLY "public"."trials"
    ADD CONSTRAINT "trials_pkey1" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."us_place_centroids"
    ADD CONSTRAINT "us_place_centroids_pkey" PRIMARY KEY ("city", "state");



ALTER TABLE ONLY "public"."user_digest_log"
    ADD CONSTRAINT "user_digest_log_pkey" PRIMARY KEY ("user_id", "sent_at");
...
CREATE INDEX "idx_geocode_cache_source" ON "public"."geocode_cache" USING "btree" ("source");
...
CREATE INDEX "idx_leads_nct_id" ON "public"."leads" USING "btree" ("nct_id");



CREATE INDEX "idx_physician_referrals_condition" ON "public"."physician_referrals" USING "btree" ("condition");



CREATE INDEX "idx_referrals_created_at" ON "public"."physician_referrals" USING "btree" ("created_at");



CREATE INDEX "idx_saved_trials_user_created" ON "public"."saved_trials" USING "btree" ("user_id", "created_at" DESC);



CREATE INDEX "idx_sites_country" ON "public"."sites" USING "btree" ("country");



CREATE INDEX "idx_sites_country_state_city" ON "public"."sites" USING "btree" ("country", "state", "city");



CREATE INDEX "idx_sites_lat_lon" ON "public"."sites" USING "btree" ("lat", "lon");



CREATE INDEX "idx_sites_latlon" ON "public"."sites" USING "btree" ("lat", "lon") WHERE (("lat" IS NOT NULL) AND ("lon" IS NOT NULL));



CREATE INDEX "idx_sites_nct" ON "public"."sites" USING "btree" ("nct_id");
...
CREATE INDEX "idx_sites_trial" ON "public"."sites" USING "btree" ("trial_id");



CREATE INDEX "idx_trial_submissions_created_at" ON "public"."trial_submissions" USING "btree" ("created_at");



CREATE INDEX "idx_trials_accessibility_tags" ON "public"."trials" USING "gin" ("accessibility_tags");



CREATE INDEX "idx_trials_condition_slugs" ON "public"."trials" USING "gin" ("condition_slugs");



CREATE INDEX "idx_trials_conditions_array" ON "public"."trials" USING "gin" ("conditions");



CREATE INDEX "idx_trials_conditions_gin" ON "public"."trials" USING "gin" ("conditions");



CREATE INDEX "idx_trials_criteria_gin" ON "public"."trials" USING "gin" ("criteria_json");



CREATE INDEX "idx_trials_criteria_json_gin" ON "public"."trials" USING "gin" ("criteria_json");



CREATE INDEX "idx_trials_gender" ON "public"."trials" USING "btree" ("gender");



CREATE INDEX "idx_trials_max_age" ON "public"."trials" USING "btree" ("max_age_years");



CREATE INDEX "idx_trials_min_age" ON "public"."trials" USING "btree" ("min_age_years");



CREATE UNIQUE INDEX "idx_trials_nct_unique" ON "public"."trials" USING "btree" ("nct_id");



CREATE INDEX "idx_trials_original_conditions" ON "public"."trials" USING "gin" ("original_conditions" "jsonb_path_ops");



CREATE INDEX "idx_trials_phase" ON "public"."trials" USING "btree" ("phase");



CREATE INDEX "idx_trials_publishable" ON "public"."trials" USING "btree" ("is_publishable");



CREATE INDEX "idx_trials_publishable_us" ON "public"."trials" USING "btree" ("status") WHERE (("is_publishable" IS TRUE) AND ("is_us" IS TRUE));



CREATE INDEX "idx_trials_quality_flags_gin" ON "public"."trials" USING "gin" ("quality_flags" "jsonb_path_ops");



CREATE INDEX "idx_trials_status" ON "public"."trials" USING "btree" ("lower"("status"));



CREATE INDEX "idx_trials_travel_stipend" ON "public"."trials" USING "btree" ("travel_stipend");



CREATE INDEX "idx_trials_visit_model" ON "public"."trials" USING "btree" ("visit_model");



CREATE INDEX "idx_us_place_centroids_state_city" ON "public"."us_place_centroids" USING "btree" ("state", "city");



CREATE INDEX "idx_zip_centroids_zip" ON "public"."zip_centroids" USING "btree" ("postal_code");
...
CREATE INDEX "sites_geom_gist" ON "public"."sites" USING "gist" ("geom");



CREATE INDEX "sites_lat_lon_idx" ON "public"."sites" USING "btree" ("lat", "lon");



CREATE INDEX "sites_nct_id_idx" ON "public"."sites" USING "btree" ("nct_id");



CREATE INDEX "sites_trial_id_idx" ON "public"."sites" USING "btree" ("trial_id");



CREATE INDEX "trials_condition_slugs_gin" ON "public"."trials" USING "gin" ("condition_slugs");



CREATE INDEX "trials_conditions_gin" ON "public"."trials" USING "gin" ("conditions");



CREATE UNIQUE INDEX "uniq_sites_trial_siteindex" ON "public"."sites" USING "btree" ("trial_id", "site_index");



CREATE OR REPLACE TRIGGER "set_updated_at" BEFORE UPDATE ON "public"."trials" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
CREATE OR REPLACE TRIGGER "set_updated_at_physician_referrals" BEFORE UPDATE ON "public"."physician_referrals" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "set_updated_at_trails" BEFORE UPDATE ON "public"."trials" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "set_updated_at_trial_submissions" BEFORE UPDATE ON "public"."trial_submissions" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "trg_profiles_updated" BEFORE UPDATE ON "public"."profiles" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at_profiles"();



CREATE OR REPLACE TRIGGER "trg_trials_phase_normalizer" BEFORE INSERT OR UPDATE OF "study_type", "phase", "interventions" ON "public"."trials" FOR EACH ROW EXECUTE FUNCTION "public"."trials_phase_normalizer"();



ALTER TABLE ONLY "public"."physician_referrals"
    ADD CONSTRAINT "physician_referrals_condition_fkey" FOREIGN KEY ("condition") REFERENCES "public"."conditions"("slug") ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY "public"."saved_trials"
    ADD CONSTRAINT "saved_trials_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."screener_sessions"
    ADD CONSTRAINT "screener_sessions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."sites"
    ADD CONSTRAINT "sites_trial_id_fkey" FOREIGN KEY ("trial_id") REFERENCES "public"."trials"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."user_digest_log"
    ADD CONSTRAINT "user_digest_log_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



CREATE POLICY "Anyone can insert a lead" ON "public"."leads" FOR INSERT TO "anon" WITH CHECK (true);



CREATE POLICY "Deny all access by default" ON "public"."conditions" TO "authenticated", "anon" USING (false);



CREATE POLICY "No direct access" ON "public"."patient_leads" USING (false) WITH CHECK (false);
...
CREATE POLICY "deny all leads" ON "public"."leads" USING (false);



CREATE POLICY "deny all physician_referrals" ON "public"."physician_referrals" USING (false);



CREATE POLICY "deny all trial_submissions" ON "public"."trial_submissions" USING (false);



CREATE POLICY "digest_self_select" ON "public"."user_digest_log" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "events_no_public" ON "public"."events" USING (false);
...
CREATE POLICY "leads_no_public" ON "public"."leads" USING (false);
...
ALTER TABLE "public"."physician_referrals" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "public can read trials" ON "public"."trials" FOR SELECT USING (true);



CREATE POLICY "public read trials" ON "public"."trials" FOR SELECT USING (true);



CREATE POLICY "saved_self_delete" ON "public"."saved_trials" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "saved_self_select" ON "public"."saved_trials" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "saved_self_upsert" ON "public"."saved_trials" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "screener_insert_any" ON "public"."screener_sessions" FOR INSERT TO "anon" WITH CHECK (true);



CREATE POLICY "screener_select_own" ON "public"."screener_sessions" FOR SELECT USING ((COALESCE("user_id", "auth"."uid"()) = "auth"."uid"()));
...
ALTER TABLE "public"."user_digest_log" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."nearest_sites"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."nearest_sites"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."nearest_sites"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."nearest_sites_with_meta"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."nearest_sites_with_meta"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."nearest_sites_with_meta"("in_lat" double precision, "in_lon" double precision, "max_miles" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."normalize_phase_value"("p" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."normalize_phase_value"("p" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."normalize_phase_value"("p" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."trials_phase_normalizer"() TO "anon";
GRANT ALL ON FUNCTION "public"."trials_phase_normalizer"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trials_phase_normalizer"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."upsert_profile"("p_user_id" "uuid", "p_profile" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."upsert_profile"("p_user_id" "uuid", "p_profile" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."upsert_profile"("p_user_id" "uuid", "p_profile" "jsonb") TO "service_role";
...
GRANT ALL ON TABLE "public"."interventional_na_ncts" TO "anon";
GRANT ALL ON TABLE "public"."interventional_na_ncts" TO "authenticated";
GRANT ALL ON TABLE "public"."interventional_na_ncts" TO "service_role";
...
GRANT ALL ON TABLE "public"."patient_leads" TO "anon";
GRANT ALL ON TABLE "public"."patient_leads" TO "authenticated";
GRANT ALL ON TABLE "public"."patient_leads" TO "service_role";



GRANT ALL ON TABLE "public"."physician_referrals" TO "anon";
GRANT ALL ON TABLE "public"."physician_referrals" TO "authenticated";
GRANT ALL ON TABLE "public"."physician_referrals" TO "service_role";



GRANT ALL ON TABLE "public"."saved_trials" TO "anon";
GRANT ALL ON TABLE "public"."saved_trials" TO "authenticated";
GRANT ALL ON TABLE "public"."saved_trials" TO "service_role";



GRANT ALL ON TABLE "public"."screener_sessions" TO "anon";
GRANT ALL ON TABLE "public"."screener_sessions" TO "authenticated";
GRANT ALL ON TABLE "public"."screener_sessions" TO "service_role";
...
GRANT ALL ON TABLE "public"."trial_submissions" TO "anon";
GRANT ALL ON TABLE "public"."trial_submissions" TO "authenticated";
GRANT ALL ON TABLE "public"."trial_submissions" TO "service_role";



GRANT ALL ON TABLE "public"."trials_display" TO "anon";
GRANT ALL ON TABLE "public"."trials_display" TO "authenticated";
GRANT ALL ON TABLE "public"."trials_display" TO "service_role";



GRANT ALL ON TABLE "public"."trials_public" TO "anon";
GRANT ALL ON TABLE "public"."trials_public" TO "authenticated";
GRANT ALL ON TABLE "public"."trials_public" TO "service_role";



GRANT ALL ON TABLE "public"."trials_quality" TO "anon";
GRANT ALL ON TABLE "public"."trials_quality" TO "authenticated";
GRANT ALL ON TABLE "public"."trials_quality" TO "service_role";



GRANT ALL ON TABLE "public"."trials_with_normalized_criteria" TO "anon";
GRANT ALL ON TABLE "public"."trials_with_normalized_criteria" TO "authenticated";
GRANT ALL ON TABLE "public"."trials_with_normalized_criteria" TO "service_role";



GRANT ALL ON TABLE "public"."us_place_centroids" TO "anon";
GRANT ALL ON TABLE "public"."us_place_centroids" TO "authenticated";
GRANT ALL ON TABLE "public"."us_place_centroids" TO "service_role";



GRANT ALL ON TABLE "public"."us_place_centroids_stage" TO "anon";
GRANT ALL ON TABLE "public"."us_place_centroids_stage" TO "authenticated";
GRANT ALL ON TABLE "public"."us_place_centroids_stage" TO "service_role";



GRANT ALL ON TABLE "public"."user_digest_log" TO "anon";
GRANT ALL ON TABLE "public"."user_digest_log" TO "authenticated";
GRANT ALL ON TABLE "public"."user_digest_log" TO "service_role";
