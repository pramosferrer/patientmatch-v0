...
This document defines the schema and data format for the **Serving Layer** (Supabase) that connects the external data factory (`ct_project`) to the storefront (`patientmatch-v0`).
...
| `display_title` | `text` | **Optional** | Patient-friendly short title. |
...
Derived patient-facing insights for trials.

| Column | Type | Description |
| --- | --- | --- |
| `nct_id` | `text` (PK) | ClinicalTrials.gov identifier. |
| `plain_summary_json` | `jsonb` | Patient-facing summary payload (display title, summary, key info). |
