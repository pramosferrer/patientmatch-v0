create index if not exists trial_sites_build_geom_idx
  on public.trial_sites using gist (geom);

create index if not exists trial_sites_build_nct_idx
  on public.trial_sites (build_tag, nct_id);

create index if not exists trials_build_status_idx
  on public.trials (build_tag, status_bucket);

create index if not exists trials_build_quality_idx
  on public.trials (build_tag, quality_score desc, nct_id);
...
nearest_per_trial as (
  ...
  from (
    select
      candidates.*,
      row_number() over (partition by nct_id order by nearest_miles asc) as rn
    from candidates
  ) ranked
  where rn = 1
),
