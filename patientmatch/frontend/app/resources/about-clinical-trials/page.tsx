      ...
      <div className="pm-container py-16 md:py-20">
        <div className="space-y-16">
              ...
              {phases.map((phase) => (
                <li key={phase.badge} className="relative flex flex-col gap-4 md:flex-1 md:gap-5">
                  <div className="flex items-start gap-4 md:flex-col md:items-start md:gap-3">
                    <span className="relative inline-flex h-12 w-12 items-center justify-center rounded-none border border-border/70 bg-background">
                      <span
                        className="pointer-events-none absolute inset-[-6px] rounded-none border border-primary/30 motion-safe:animate-[pulse_4s_ease-in-out_infinite] motion-reduce:animate-none"
                        aria-hidden="true"
                      />
          ...
          <MotionSection id="types" className="space-y-8">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {typeTiles.map((tile) => (
                <article
                  key={tile.name}
                  className="flex flex-col gap-4 rounded-none border border-border/60 bg-background/70 p-6 shadow-sm/5"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tile.examples.map((example) => (
                      <span
                        key={example}
                        className="inline-flex items-center rounded-none border border-border/60 bg-muted/40 px-3 py-1 text-xs font-medium text-muted-foreground"
                      >
                        {example}
                      </span>
                    ))}
          ...
          <MotionSection id="safety" className="space-y-6">
            ...
            <ul className="grid gap-4 md:grid-cols-2">
              {safetyItems.map((item) => (
                <li
                  key={item.lead}
                  className="flex items-start gap-3 rounded-none border border-border/50 bg-muted/30 p-5 text-sm leading-relaxed text-muted-foreground md:text-base"
                >
                  {item.icon}
                  <span>
          ...
          <MotionSection id="costs" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {costItems.map((item) => (
                <article
                  key={item.headline}
                  className="flex items-start gap-4 rounded-none border border-border/50 bg-background/70 p-5 text-sm leading-relaxed text-muted-foreground md:text-base"
                >
                  {item.icon}
                  <div>
          ...
          <MotionSection id="why" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {whyItems.map((item) => (
                <div
                  key={item.headline}
                  className="flex items-start gap-4 rounded-none border border-border/50 bg-muted/30 p-5"
                >
                  {item.icon}
          ...
          <MotionSection id="faq" className="space-y-6">
            ...
            <div className="divide-y divide-border overflow-hidden rounded-none border border-border/60 bg-background/70">
              {faqs.map((faq, index) => (
            ...
            <div className="flex flex-wrap gap-3">
              {glossary.map((item) => (
                <abbr
                  ...
                  className="inline-flex cursor-help items-center rounded-none border border-border/60 bg-muted/30 px-3 py-1 text-xs font-medium text-muted-foreground"
                >
                  {item.term}
                </abbr>
              ))}
