...
export function getConditionIcon(slug?: string | null): { Icon: IconType; className: string } {
  ...
  if (!slug) {
    console.log("getConditionIcon: no slug, returning fallback");
    return fallback;
  }
  
  const result = ICONS[slug] ?? fallback;
  console.log(`getConditionIcon: slug="${slug}", found=${!!ICONS[slug]}, returning:`, result);
  return result;
}
