...
export function guessConditionSlug(
  ...
  // Debug logging for trials that might not match
  if (!title && (!originals || originals.length === 0)) {
    console.log("No title or original conditions to guess from");
    return null;
  }

  for (const [slug, words] of Object.entries(KEYWORDS)) {
    if (words.some(w => hay.includes(w))) {
      console.log(`Matched "${slug}" from hay: "${hay}"`);
      return slug;
    }
  }
  
  console.log(`No match found for hay: "${hay}"`);
  return null;
}
