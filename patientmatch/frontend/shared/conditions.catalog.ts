...
async function buildConditionCatalog(options: CatalogOptions = {}): Promise<ConditionCatalog> {
  ...
  console.log('🏗️ Building condition catalog...');
  ...
  try {
    ...
    console.log(`📊 Found ${slugCounts.size} distinct condition slugs across ${totalTrials} trials (${recruitingTrials} recruiting)`);
    ...
    // Sort: Featured first (in seed order), then A-Z by label
    all.sort((a, b) => {
    ...
    console.log(`✅ Built catalog: ${featured.length} featured, ${all.length} total, ${filtered.length} filtered (minCount: ${minCount})`);
    console.log(`🎯 DB-sourced conditions: ${all.filter(c => c.source === 'db').length}`);
...
export async function revalidateConditions() {
  console.log('🔄 Revalidating condition catalog...');
