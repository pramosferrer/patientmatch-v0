...
import { evaluateTrial } from '@/lib/matching/evaluator';
import { getChipClasses } from '@/shared/colors';

export default function Shortlist({ trials, onBack, onSelect, patientProfile, onError }) {
  const [loading, setLoading] = useState(false);
  const [email, setEmail] = useState("");
  const [displayTrials, setDisplayTrials] = useState(() => trials.map((t, i) => ({ ...t, __idx: i })));
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      try {
        const evaluated = trials.map((trial, i) => {
            ...
            <div className="mt-auto">
              <a 
                href={screenerHref({ nct_id: t.nct_id }, {
                  condition: patientProfile.conditions?.[0],
                  age: patientProfile.age,
                  sex: patientProfile.sex,
                  zip: patientProfile.location?.zip,
                  miles: patientProfile.willingness_to_travel_miles
                })}
