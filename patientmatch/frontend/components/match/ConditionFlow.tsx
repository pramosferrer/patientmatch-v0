...
export default function ConditionFlow({ onShortlist, patientProfile }) {
  ...
  const selectCondition = (selectedCondition) => {
    ...
    if (selectedCondition.count === 0) {
      ...
      setTimeout(() => setShowEmptyWarning(false), 5000); // Hide after 5 seconds
    } else {
  ...
  const handleShortlist = async () => {
    ...
    try {
      ...
      const errorMessage = error.message || "Failed to find trials. Please check your connection and try again.";
