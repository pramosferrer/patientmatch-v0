        ...
        <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
          {FEATURED.map((c) => {
            ...
            return (
              <article
                key={c.slug}
                className="group relative h-full rounded-lg bg-warm-cream/85 p-5 shadow-card transition hover:-translate-y-1 hover:shadow-aurora"
              >
                <div className="flex items-start gap-4">
                  <span className={`rounded-lg ${theme.bg} ${theme.text} p-2 shadow-inner`}>
                ...
                <Link href={`/trials?condition=${c.slug}`} className="mt-4 inline-flex">
