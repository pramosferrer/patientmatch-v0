...
export default function TrialScreenClient({
  ...
  const [perspectiveChosen, setPerspectiveChosen] = useState<boolean>(false);
  ...
  const screenerConfidence =
    typeof trial.questionnaire_json?.screener_confidence === "string"
      ? trial.questionnaire_json.screener_confidence
      : null;
  ...
  if (answers && evaluation) {
    return (
      <ScreenResult
        ...
        screenerConfidence={screenerConfidence}
      />
    );
  }
  ...
  // Step 0: Who are you screening for?
  if (!perspectiveChosen) {
    return (
      <div className="rounded-xl border border-border/60 bg-white p-7 space-y-6 shadow-sm">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground/60 mb-2">Before we begin</p>
          <h2 className="text-[19px] font-semibold text-foreground leading-snug">Who are you filling this out for?</h2>
          <p className="mt-1.5 text-sm text-muted-foreground">Your answer affects how the questions are phrased.</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => { setForSelf(true); setPerspectiveChosen(true); }}
            className="group flex flex-col items-center gap-3 rounded-xl border-2 border-primary/20 bg-primary/5 p-5 text-center transition-all hover:border-primary/50 hover:bg-primary/8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            <User className="h-7 w-7 text-primary/50 transition-colors group-hover:text-primary/80" />
            <span>
              <span className="block font-semibold text-foreground text-[14.5px]">Myself</span>
              <span className="block text-xs text-muted-foreground mt-0.5">I&apos;m looking for trials for me</span>
            </span>
          </button>
          <button
            type="button"
            onClick={() => { setForSelf(false); setPerspectiveChosen(true); }}
            className="group flex flex-col items-center gap-3 rounded-xl border-2 border-border/40 bg-white p-5 text-center transition-all hover:border-primary/40 hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            <Users className="h-7 w-7 text-muted-foreground/50 transition-colors group-hover:text-primary/70" />
            <span>
              <span className="block font-semibold text-foreground text-[14.5px]">Someone I care for</span>
              <span className="block text-xs text-muted-foreground mt-0.5">A family member or loved one</span>
            </span>
          </button>
        </div>
      </div>
