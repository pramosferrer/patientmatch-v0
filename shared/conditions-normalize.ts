...
const ALIASES: Record<string, string> = {
  // Legacy slug forms emitted by older pipelines
  ...
  'diabetes type 2': 'type_2_diabetes',
  'diabetes dm': 'type_2_diabetes',
  'diabetes complications': 'type_2_diabetes',
  'diabetes type ii': 'type_2_diabetes',
  'diabetic': 'type_2_diabetes',
  'diabetics': 'type_2_diabetes',
  // Diabetes T1
  'type 1 diabetes': 'type_1_diabetes',
  'type i diabetes': 'type_1_diabetes',
  't1dm': 'type_1_diabetes',
  'juvenile diabetes': 'type_1_diabetes',
  ...
  'htn': 'hypertension',
  // Obesity - consolidate variants
  'obesity and obesity related medical conditions': 'obesity',
  'obesity and obesity-related medical conditions': 'obesity',
  ...
  // Dementia (map to Alzheimer's for now)
  'dementia': 'alzheimers_disease',
  'mild cognitive impairment': 'alzheimers_disease',
};

// Extended synonyms list - used for expanding slug queries
const NORMALIZED: Record<string, string[]> = {
  ...
  // Obesity variants all map to 'obesity' via ALIASES now
  obesity: ['overweight'],
};
...
export function toConditionSlug(input: string | null | undefined): string {
  ...
  const dashed = cleaned.replace(/_/g, '-').replace(/\s+/g, '-');
  const compact = dashed.replace(/[^a-z0-9]/g, '');
  ...
  if (TERM_TO_SLUG_EXTENDED[dashed]) return TERM_TO_SLUG_EXTENDED[dashed];
  if (TERM_TO_SLUG_EXTENDED[compact]) return TERM_TO_SLUG_EXTENDED[compact];
  ...
  // diabetes patterns
  if (
    /(^|\b)(type\s*2|type\s*ii|t2|t2dm)(\b|$).*\b(diabet\w*|dm)\b/.test(joined)
    || /\b(diabet\w*|dm)\b.*(^|\b)(type\s*2|type\s*ii|t2|t2dm)(\b|$)/.test(joined)
  ) {
    return 'type_2_diabetes';
  }
  if (
    /(^|\b)(type\s*1|type\s*i|t1|t1dm)(\b|$).*\b(diabet\w*|dm)\b/.test(joined)
    || /\b(diabet\w*|dm)\b.*(^|\b)(type\s*1|type\s*i|t1|t1dm)(\b|$)/.test(joined)
  ) {
    return 'type_1_diabetes';
  }
  if (/\b(juvenile|insulin dependent)\b.*\bdiabet\w*\b/.test(joined)) return 'type_1_diabetes';
  if (/\b(obesity|obese|overweight)\b/.test(joined)) return 'obesity';
  if (/\b(diabet\w*|dm)\b/.test(joined)) return 'type_2_diabetes';
